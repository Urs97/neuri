...
exports.onCreatePage = ({ page, actions }) => {
  ...
  // Grab the keys ('en' & 'hr') of locales and map over them
  Object.keys(locales).map(lang => {
    ...
    return createPage({
      ...
      ...page,
      // Since page.path returns with a trailing slash (e.g. "/hr/")
      // We want to remove that (e.g. "hr/")
...
exports.onCreateNode = ({ node, actions, getNode }) => {
  ...
  if (node.internal.type === `MarkdownRemark`) {
    ...
    // createNodeField({ node, name: `locale`, value: localeTest });
